...
//                        p.$jot('.jt-content, .jt-title').css('width', 'auto');
    ...
    checkMultipleUploads: function(){
        if($$('.form-upload-multiple').length > 0){
            var script = document.createElement('script');
            script.type="text/javascript";
            script.src = "https://static-interlogyllc.netdna-ssl.com/file-uploader/fileuploader.js?v7";
            $(document.body).appendChild(script);
        }
    },
    /**
    ...
    */
        ...
        var displayDuration = function() {
            if($('input_'+id+'_hourSelectRange')) {
                ...
                if(sHour.length > 0 && sMin.length > 0 && eHour.length > 0 && eMin.length > 0) {
                    ...
                    $$('input[id=duration_'+id+lab+'][type="hidden"]').first().setValue(hours+':'+min);
                } else {
    ...
    /**
     * load current date to datetime fields 
     * if element is set to read only
     */
    currentDateReadonly: function(id){
        try{
            if(! $('day_' + id).hasClassName('noDefault')){
                var date = new Date();
    ...
    calendarClose: function(calendar){
        ...
        var incomplete = calendarFields.any(function(c) { return c.value.empty()});
...
{       // Event simulator
    ...
    showWidget: function(id) {
        ...
        if(referrer && frame) {
            ...
            if ( typeof window.JCFServerCommon !== 'undefined' ) {
                ...
                if ( JotForm.isVisible(JotForm.getSection($("id_" + id) ) ) ) {
                    ...
                    case "radio":
                        ...
                        $$('#id_'+term.field+' input').each(function(input){
                            ...
                            if(JotForm.checkValueByOperator(term.operator, term.value, value)){
                                ...
                                if (input.value == term.value || (input.hasClassName('form-'+fieldType+'-other') && term.value == '-- Other --')) {
                                    all = false;
                                }
                            }
                        });
                ...
                setTimeout(function(calc) {
                    ...
                    if(!matchForThisResult) {
                        ...
                        if(["address", "combined", "datetime", "time"].include(type)) {
                            ...
                            if($('input_' + resultField + '_full') && $('input_' + resultField + '_full').readAttribute("masked") == "true") {
                                JotForm.setQuestionMasking("#input_"+resultField+"_full", "textMasking", $('input_' + resultField + '_full').readAttribute("maskValue"));
                            }
                        } else if($('id_'+resultField).readAttribute('data-type') == "control_paypalpro" || $('id_'+resultField).readAttribute('data-type') == "control_authnet") {
                            $('id_'+resultField).select('input[class*=" form-address-"], select[class*=" form-address-"]').each(function(el) {
                                el.setValue("");
                            });
                        } else if(type === "html") {
                            try {
                            ...
                            if($('input_' + resultField)) {
                                ...
                                if($('input_' + resultField).readAttribute("masked") == "true") {
                                    JotForm.setQuestionMasking("#input_"+resultField, "textMasking", $('input_' + resultField).readAttribute("maskValue"));
                                }
                            }
                        }
                    }
    ...
    */
    requireField: function(qid, req) {
        ...
        $$('#id_' + qid + ' input, #id_' + qid + ' textarea, #id_' + qid + ' select').each(function(el) {
            ...
            if(el.id === 'coupon-input' 
                ...
                || $A(['prefix', 'middle', 'suffix', 'addr_line2', 'state']).any(function(name) {return el.name.indexOf("["+name+"]") > -1;})) {
                return;
            }
    ...
    checkCalculation: function(calc) {
        ...
        var calculationResultType = function(id) {
            if($('id_'+id) && $('id_'+id).readAttribute('data-type') == "control_authnet") {
                return "authnet"
            } else if($('id_'+id) && $('id_'+id).readAttribute('data-type') == "control_paypalpro") {
                return "paypalpro";
            } else {
        ...
        try {
            if(!['text', 'email', 'textarea', 'calculation', 'combined', 'address', 'datetime', 'time', 'html', 'authnet', 'paypalpro'].include(calculationResultType(result))) return;
        } catch(e) {
            console.log(e);
        }
            ...
            for(var i = 0; i < equation.length; i++) {
                ...
                if(character === '[' && !numeric) {
                    ...
                    try {
                        ...
                        if(num) {
                            if(num.replace(/[0-9\.,]/g, "").length > 0) { //normal calc string
                            ...
                            } else { //comma separated "number" string
                                if(num.indexOf(".") > -1) {
                                    var decimalPart = 0 + num.substr(num.lastIndexOf("."));
                                    var multi = Math.pow(10, calc.decimalPlaces);
                                    decimalPart = (Math.round(decimalPart * multi)/multi).toFixed(calc.decimalPlaces);

                                    if(!calc.showEmptyDecimals) {
                                        decimalPart = parseFloat(decimalPart).toString();
                                    }

                                    var wholePart = num.substr(0, num.lastIndexOf("."));
                                    num = wholePart + decimalPart.substr(1);
                                }
                            }
                            out += num;
                        }
        ...
        var resultFieldType = calculationResultType(result);
    ...
    * @param {type} text
    ...
    isPaymentSelected: function () {
        ...
        // if this is a hidden single item     
            ...
            selected = $$('input[name*="q' + paymentFieldId +'"][type="hidden"]').length > 0;
        }
        return selected;
    },
        ...
        $H(prices).each(function(pair){
            ...
            if ($(pair.value.specialPriceField)) { 
                ...
                if(pair.value.child && pair.value.specialPriceField.split("_").length === 4){
                    ...
                    // for subproducts with special quantity, the default quantity is 0 instead of 1
                    var priceIndex = isNaN($(specialPriceField).options[0].value) || $(specialPriceField).options[0].value > 0 ? specialPriceField.getSelected().index : specialPriceField.getSelected().index - 1;
                    if (priceIndex > -1) {
                        ...
                        var checkValues = cont.select('input,select[name*=' + input.name.replace(/\[.*$/, '') + ']').map(function(e){
                            ...
                            if (e.id.match(/input_[0-9]+_quantity_[0-9]+_[0-9]+/) && e.type == 'text') {
                                ...
                                if ((cb.checked && !subProductQty && (isNaN(e.value) || e.value == 0 || e.value.empty())) // if a product is selected and qty is not valid
                                     || (!allProducts.map(function(c) { return c.checked }).any()) // if there are no products selected
                                     || (cb.checked && subProductQty && !subProductQty.map(function(q) { return q.value > 0 }).any()) // if this is a subproduct and none of the subproduct quantity are filled
                                ) {
